...
func TestMap(t *testing.T) {
	...
	var (
		err error
		stm types.StateMachine[Entry, map[string]uint32]
	)
	...
	t.Run("Execute", func(t *testing.T) {
		t.Run("SupportedCommands", func(t *testing.T) {
			setup(t)
			input := Entry{"elite", 0x1337}

			t.Run("Put", func(t *testing.T) {
				err := stm.Execute(types.PutCommand, input)
				assert.NoError(t, err)
				assert.Equal(t, map[string]uint32{input.Key: input.Value}, stm.State())
			})
			...
			t.Run("Delete", func(t *testing.T) {
				...
				assert.NoError(t, err)
				assert.Equal(t, map[string]uint32{}, stm.State())
			})
		})
	...
	t.Run("State", func(t *testing.T) {
		input := map[string]uint32{"key": 0}
		opt := statemachineadapter.WithInitialState[Entry](input)

		stm, err := statemachineadapter.NewMap(transformFunc, opt)
		assert.NoError(t, err)
	...
	t.Run("DeepCopy", func(t *testing.T) {
		...
		inputStm, err := statemachineadapter.NewMap(transformFunc, opt)
		assert.NoError(t, err)
		...
		// -- States are mutually immutable
		err = actual0.Execute(types.PutCommand, Entry{"test", 1337})
		assert.NoError(t, err)
		err = actual1.Execute(types.DeleteCommand, Entry{"key", 1111})
		assert.NoError(t, err)
